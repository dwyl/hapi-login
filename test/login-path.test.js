...
var opts   = { fields:fields, handler:handler, loginPath: loginPath };
...
/************************* TESTS ***************************/
test(file+'Attempt to submit a /api/login request without password', function(t){
  var options = {
    method: "POST",
    url: "/api/login",
  ...
  server.inject(options, function(response) {
    ...
    t.equal(code, 400, '/api/login without password fails -> '+code);
    server.stop(function(){ t.end(); });
  });
});

test(file+'Attempt to /api/login with unrecognised field', function(t){
  var options = {
    method: "POST",
    url: "/api/login",
  ...
  server.inject(options, function(response) {
    // joi returns 400 when auth validation fails.
    var code = response.statusCode;
    t.equal(code, 400, '/api/login with unknown field fails -> '+code);
    server.stop(function(){ t.end(); });
  });
});

test(file+"Subit /api/login with incorrect password", function(t) {
  var person = {
    "email"    : 'john@smith.net',
    "password" : 'incorrect'
  };
  ...
  server.inject(options, function(response) {
    t.equal(response.statusCode, 400, "/api/login failed when  password incorrect");
    server.stop(function(){ t.end(); });
  });
});

test(file+"Successfully /api/login with email and password", function(t) {
  var person = {
    "email" : 'john@smith.net',
    "password":'secret'
  };
  ...
  server.inject(options, function(response) {
    ...
    t.equal(response.statusCode, 200, "/api/login worked with email and password");
    server.stop(function(){ t.end(); });
  });
});

test(file+"Default route GET requests should fail", function(t) {
  var options = {
    method: "GET",
    url: "/login"
  };
  ...
  server.inject(options, function(response) {
    t.equal(response.statusCode, 404, "/login should return a 404 not found status");
    server.stop(function(){ t.end(); });
  });
});

test(file+"Default route POST requests should fail", function(t) {
  var person = {
    email: "john@smith.net",
    password: "secret"
  };
  ...
  server.inject(options, function(response) {
    t.equal(response.statusCode, 404, "/login should return 404 not found status");
    server.stop(function() { t.end(); });
  });
});
